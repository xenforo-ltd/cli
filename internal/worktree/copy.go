...
func copyFile(src, dst string, mode os.FileMode) error {
	...
	// A destination that is already a symlink would be followed, writing
	// through it and truncating whatever it points at -- which for a link back
	// into the source tree means destroying the original. Replacing the entry
	// rather than opening it keeps the write inside the destination.
	_ = os.Remove(dst)
...
func copySymlink(src, dst string) error {
	...
	// The target is reproduced exactly, including an absolute one that points
	// back into the source tree. That is deliberate: these links are usually
	// deliberate references to a shared asset store or cache, and rewriting
	// them to follow the copy would break exactly the sharing they exist for.
	// The cost is that such a link in a cloned worktree still resolves to the
	// source, which is why copyFile replaces its destination rather than
	// writing through one.
